...
void loop()
{
  ...
  for (uint8_t i = 0; i < SensorCount; i++)
  {
    ...
    if (sensorValues[0] <= 500){
      auto mapValue = map(sensorValues[0], 0, 500, 60, 89);
      s1.write(mapValue);
    } else if (sensorValues[1] <= 500 && sensorValues[2] <= 500){
      // auto mapValue = map(sensorValues[1], 0, 500, 90, );
      s1.write(90);
    }else if (sensorValues[3] <= 500){
      auto mapValue = map(sensorValues[1], 0, 500, 90, 120);
      s1.write(mapValue);
    } else continue;
  }
